...
for dat_file in dat_files:
    ...
    for K in range(1,Loopsize+1):
        ...
        model.degra =Param(default = 0.02)
        # model.Cost_SOC =Param(default=20)
        model.ESSE = Param(default=0.9)
        model.ESSFeedBackLimit = Param(default = 1000*pow((1-ReducePercentage),(K-1))) #*12 for 2 week 
        ...
        def objfunction(model):
            ...
            totalcost = sum(model.gen_Kom[i]*(model.Pg[i,j] - model.gen_min[i])+ model.gen_NlCost[i]*model.u[i,j]+ model.gen_SuCost[i]*model.v[i,j] for i in model.GEN for j in model.PERIOD) + sum(model.BUYFGRID[j]*model.PgridCost[j]-model.SELLTGRID[j]*model.PgridCost[j]*model.SelltoGrid for j in model.PERIOD) + sum(model.PCHARGE[j]*model.ESSE*model.degra + model.PDISCHARGE[j]/model.ESSE*model.degra for j in model.PERIOD)
            return totalcost
        ...
        # def Essend(model, j):
        #     expr = model.ESS[24]   # Adjusted by the ending of time period
        #     return expr == model.ESSinitial
        # model.Cons_Essend = Constraint(model.PERIOD, rule = Essend)
        
        def Essend(model, j):
        # This constraint should apply only at the end of each day.
            if j % 24 == 0:  # Checks if j is a multiple of 24 (i.e., the end of each day)
                return model.ESS[j] == model.ESSinitial
            else:
        ...
        SCUC_instance = model.create_instance(dat_file)
        ...
        for j in SCUC_instance.PERIOD:
            total_pdischarge += SCUC_instance.PDISCHARGE[j].value
            total_pcharge += SCUC_instance.PCHARGE[j].value
        print((total_pdischarge +  total_pcharge)*0.02)
        ...
        for k, g in groupby(enumerate(data), lambda i: i[0] - i[1]):
            ...
            #print(Average)
            if group[0] == 0:
                input_x1[0,0] = Capacity
                input_x1[0,1] = 25/32
                input_x1[0,2] = 0.667
                input_x1[0,3] = SUM/TotalCap
                input_x1[0,4] = Average/TotalCap/5
            else:
                input_x1[0,0] = Capacity
                input_x1[0,1] = 25/32
                input_x1[0,2] = float(Data.iloc[int(group[0])-1]['SOC'])
                input_x1[0,3] = SUM/TotalCap
                input_x1[0,4] = Average/TotalCap/5
            x_tensor = torch.Tensor(input_x1).unsqueeze(0).cuda()
            hidden = None
            #test_out, test_h = load_rnn(x_tensor, hidden)
            IR_Output = DNN1_IR(x_tensor)
            Cycle_Num_Output = DNN1_CycleNum(x_tensor)
            IR = IR_Output.cpu().data.numpy().flatten()
            Cycle_Num = Cycle_Num_Output.cpu().data.numpy().flatten()
            if group[0] == 0:
                input_x2[0,0] = IR_Output
                input_x2[0,1] = 25/32
                input_x2[0,2] = 0.667
                input_x2[0,3] = SUM/TotalCap
                input_x2[0,4] = Average/TotalCap/5
                input_x2[0,5] = Capacity
                input_x2[0,6] = Cycle_Num_Output
            else:
                input_x2[0,0] = IR_Output
                input_x2[0,1] = 25/32
                input_x2[0,2] = float(Data.iloc[int(group[0])-1]['SOC'])
                input_x2[0,3] = SUM/TotalCap
                input_x2[0,4] = Average/TotalCap/5
                input_x2[0,5] = Capacity
                input_x2[0,6] = Cycle_Num_Output
            x_tensor = torch.Tensor(input_x2).unsqueeze(0).cuda()
            test_out = DNN2_06(x_tensor)
    ...
    Allresult.append(outputresult)
    TotalDegradation = []
    DegradationCost = []
    OperationalCost = []
    RemainingCapacity = []

df_rows = pd.DataFrame(Allresult, columns=['TotalCost[min_index]', 'DegradationCost[min_index]', 'DecreaseTotalCost', 'DecreaseTotalDegradation', 'IncreaseOperationCost', 'end - start'])    
df_cleaned = df_rows.applymap(remove_brackets)